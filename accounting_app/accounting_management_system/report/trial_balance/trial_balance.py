...
from accounting_app.accounting_management_system.report.balance_sheet.balance_sheet import filter_accounts, set_gl_entries_by_account
...
def get_data(filters):
	accounts = frappe.db.sql("""select name, account_number, parent_accounts, account_name, account_type, lft, rgt
		from `tabAccounts` order by lft""", as_dict=1)
	if not accounts:
		return None	
...
def get_opening_balances(filters):
	...
	gle = frappe.db.sql("""
		...
		group by account""", query_filters , as_dict=1)
